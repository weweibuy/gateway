...
create table gw_gateway_router
(
   id                   bigint unsigned not null auto_increment comment 'id',
   router_id             varchar(30) not null comment '路由id',
   system_id            varchar(30) not null default '' comment '路由系统id',
   system_name          varchar(50) not null default '' comment '路由系统名称',
   router_uri            varchar(50) not null default '' comment 'uri',
   router_priority       int not null default 0 comment '优先级',
   is_delete            tinyint(1) unsigned not null default 0 comment '是否删除',
   create_time          timestamp not null default CURRENT_TIMESTAMP comment '创建时间',
   update_time          timestamp not null default CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP comment '更新时间',
   primary key (id),
   unique key uk_router_id (router_id)
) ENGINE=INNODB DEFAULT CHARSET=utf8mb4 COMMENT='路由表';



create table gw_router_predicate
(
   id                   bigint unsigned not null auto_increment comment 'id',
   router_id             varchar(30) not null default '' comment 'router_id 关联路由表',
...
create table gw_router_predicate_args
(
   id                   bigint unsigned not null auto_increment comment 'id',
...
create table gw_router_filter
(
   id                   bigint unsigned not null auto_increment comment 'id',
   router_id             varchar(30) not null default '' comment '关联路由表 router_id',
...
create table gw_router_filter_args
(
   id                   bigint unsigned not null auto_increment comment 'id',
